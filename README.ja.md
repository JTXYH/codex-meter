...
- 今日の内訳、昨日、過去 7 日間、累計 Token、連続使用日数、最長タスクを表示
- 曜日ごとに整列したコンパクトなマスで過去 120 日間を表示し、ポインターを置くと日ごとの使用量を確認可能
- 今日の Token をローカル Codex セッションログから5秒ごとに差分更新し、入力、出力、キャッシュ入力、米ドルの API 換算料金を表示
- Codex Credits 残高を米ドルに換算して表示
- すべてのダッシュボードカードを個別に表示・非表示、ドラッグ並べ替えでき、設定は自動保存
- 手動更新、プリセット、1〜1,440分のカスタム更新間隔
- ログイン時の自動起動と、システム、ライト、ダークの外観
...
[⬇️ Codex Meter v1.5.0 をダウンロード（macOS Universal 2）](https://github.com/JTXYH/codex-meter/releases/download/v1.5.0/CodexMeter-1.5.0-macOS.zip)

Apple Silicon と Intel Mac の両方に対応しています。ZIP を解凍し、`CodexMeter.app` を「アプリケーション」フォルダに移動してください。[v1.5.0 のリリースノート](https://github.com/JTXYH/codex-meter/releases/tag/v1.5.0)。
...
6. 左下の歯車から設定を開き、ログイン時の起動、外観、言語、カードの表示と順序、割り当て背景、自動更新間隔を変更できます。
7. 右下の電源ボタンで終了します。

## データとプライバシー

- アカウント概要は `account/read` から取得します。
- 割り当てウィンドウと Credits 残高は `account/rateLimits/read` から取得します。パーセントは各ウィンドウの使用済み割合を表し、Credits はサーバーが返した残高を米ドルに換算します。
